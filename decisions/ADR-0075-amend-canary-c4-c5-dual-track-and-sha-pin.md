...
canary 连续红（#257）的两处预期勘误：C4 供应链入口断言由"@vN 单一形态"改为承认双合法形态（@vN | 40-hex SHA+ciw-ref 透传），与 C6 钉扎政策一致；C5 main-protection required checks 预期由单轨 ['gate'] 更新为双轨 ['gate','org-gate']（BP-2 观察期设计，退役本地轨需未来新 ADR）。选型倾向 SHA-only（不可变+可审计），但 @vN 仍合法——不断言形态统一。
